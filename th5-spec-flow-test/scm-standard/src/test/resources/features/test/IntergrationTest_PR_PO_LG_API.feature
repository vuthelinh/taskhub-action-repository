@vincent @angela @scm @intergration_pr_po_lg_lgapi
Feature: Intergration Test - PR-PO-LG-LGAPI
  ...
  Scenario Outline: Intergration Test - PR-PO-LG-LGAPI
    ...
    Then user checks number of records in database as "<SQL Statement KEY>"
      ...
      | 001 | select count(*) from gl_ledger_summary where source_voucher_no = '@@#1#@@' | 3       | Shipment Voucher No    | -1      |                        |         |
      | 001 | select count(*) from gl_ledger_summary where source_voucher_no = '@@#1#@@' | 2       | Invoice No             |         |                        |         |

    Examples: 
      | Purchase Requisition KEY | Purchase Order KEY | Approving Officer KEY | Approval Summary KEY | Combine PR KEY | Billing Purchase KEY | SQL Statement KEY |
