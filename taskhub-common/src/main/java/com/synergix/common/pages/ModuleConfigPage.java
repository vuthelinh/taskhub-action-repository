...
public class ModuleConfigPage extends TH5PageObject {
	...
	public void set_module_config(String _moduleConfigName, int _moduleConfigIndex, String _configType,
		...
		switch (_configType) {
		case "Radio":
			selectRadioButton(xPathRadioBtnWithGroupLabel("", _moduleConfigName, _moduleConfigIndex, _configValue));
			break;
		case "Input":
			enterInputFieldWithLabel(_moduleConfigName, _configValue, _moduleConfigIndex);
			break;
		case "Select":
			selectDdlWithLabelByText(_moduleConfigName, _moduleConfigIndex, _configValue);
			break;
		case "Search":
			try {
				String[] asConfigValue = _configValue.split(">");
				clickSearchIconWithLabel("", _moduleConfigName, _moduleConfigIndex);
			...
			break;
		case "Remarks":
			enterRemarkByClickIcon(xPathRemarksWithLabel("", _moduleConfigName, _moduleConfigIndex, 1), _configValue);
			break;
		default:
	...
	public void switch_to_module_config_section2(String moduleConfigTabPath) {
		...
		for (int i = 0; i < size; i++) {
			int findVoucher = findAll("//td[contains(text(),'" + asTabPath[i]
					+ "') and contains(@class,'rich-tab-header rich-tab-inactive')]").size();
			LogWork.log_debug("findVoucher: " + findVoucher);
			if (findVoucher != 0) {
				$("//td[contains(text(),'" + asTabPath[i]
						+ "') and contains(@class,'rich-tab-header rich-tab-inactive')]").waitUntilClickable().click();
