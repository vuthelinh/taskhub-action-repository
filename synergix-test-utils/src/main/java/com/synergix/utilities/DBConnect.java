...
//            }
    ...
    // check column value if it returns the same result as 'value' parameter, filePath for detailed output results
                                           ...
                                           String filePath, String sheetName, boolean printAllResult)
    {
        ...
        if (columnValue != null)
        {
            ...
            {
                if (check != true)
                    //ReadBean.writeToFile(filePath, connectionString + "     " + query + "     " + columnName
                    //+ " Expected result: " + value + " Actual result: " + columnValue + " Fail");
                    testResult = "Failed";
            }
            else
            {
                if (check != true)
                    //ReadBean.writeToFile(filePath, connectionString + "     " + query + " " + columnName
                    //+ " Expected result: " + value + " Actual result: " + columnValue + " Failed");
                    testResult = "Failed";
                else
                    //ReadBean.writeToFile(filePath, connectionString + "     " + query + " " + columnName
                    //+ " Expected result: " + value + " Actual result: " + columnValue + " Passed");
                    testResult = "Passed";
            }
        }
        else if (columnValue == null && ((value.toString().equals("")) || value.equalsIgnoreCase("null")))
        {
            ...
            //value = ReadBean.addAlignedSpace(value, 5);
            //ReadBean.writeToFile(filePath,
            //connectionString + "     " + query + " " + columnName + "   Execution passed " + "   Passed");
            testResult = "Passed";
            check = true;
        }
        else
        {
            ...
            //value = ReadBean.addAlignedSpace(value, 5);
            //ReadBean.writeToFile(filePath,
            //connectionString + "     " + query + " " + columnName + "   Execution failed " + "   Failed");
    ...
    public static boolean checkBooleanList(ArrayList<Boolean> list)
    {
        ...
        if (result == list.size())// all correct
            return true;
