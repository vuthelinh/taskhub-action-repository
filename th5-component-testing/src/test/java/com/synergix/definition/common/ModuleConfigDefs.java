...
public class ModuleConfigDefs {
	...
	@Then("^user verify text prensent in \"([^\"]*)\" module config$")
	public void user_verify_text_Module_Config(String ModuleConfigName, List<List<String>> dataTable) {
		...
		for (int row : SessionData.getDataTbRowsNoHeader("MODULE_CONFIG").keySet()) {
			...
			sModuleSection = ModuleConfigName + ">" + sModuleSection;

			String sModuleName = SessionData.getDataTbVal("MODULE_CONFIG", row, "Module Name");

			moduleConfigSteps.switch_to_module_config_section2(sModuleSection);

			String matcher = moduleConfigSteps.get_text_label(row);
			if (!sModuleName.equals(matcher))
				LogWork.log_debug("Assertion error: Expected: is [" + sModuleName + "] but was [" + matcher + "]");
		}
	}
